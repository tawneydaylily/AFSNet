...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        # run the mdoel
        output, output2, output3, output4, output5 = model(pre_img_var, post_img_var)
        loss = BCEDiceLoss(output, target_var) + BCEDiceLoss(output2, target_var) + BCEDiceLoss(output3, target_var) + \
               BCEDiceLoss(output4, target_var) + BCEDiceLoss(output5, target_var)
...
def train(args, train_loader, model, optimizer, epoch, max_batches, cur_iter=0, lr_factor=1.):
    ...
    for iter, batched_inputs in enumerate(train_loader):
        ...
        img, target = batched_inputs
        pre_img = img[:, 0:3]  #transform里面有个totensor B C H W
        post_img = img[:, 3:6]
        ...
        # run the model
        output, output2, output3, output4, output5 = model(pre_img_var, post_img_var)
        loss = BCEDiceLoss(output, target_var) + BCEDiceLoss(output2, target_var) + BCEDiceLoss(output3, target_var) + BCEDiceLoss(output4, target_var) + BCEDiceLoss(output5, target_var)
...
def trainValidateSegmentation(args):
    ...
    if args.file_root == 'LEVIR':
        args.file_root = 'datasets/LEVIR-CD'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_steps', type=int, default=70000, help='Max. number of iterations')
